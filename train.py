...
def main(args):
    ...
    train_loader, valid_loader = data_loaders(args)
    ...
    for epoch in tqdm(range(args.epochs), total = args.epochs):
        ...
        for i, data in enumerate(loaders[phase]):
            ...
            x, y_true = data
            x, y_true = x.to(deivce), y_true.to(device)
                ...
                if phase == "valid":
                    ...
                    if (epoch % args.vis_freq == 0) or (epoch == args.epochs - 1):
                        if i * args.batch_size < args.vis_images:
                            ...
                            logger.image_list_summary(tag,log_images(x, y_true, y_pred)[:num_images],step,)
...
def data_loaders(args):
    train_dataset, valid_dataset = datasets(args)
    ...
    valid_loader = DataLoader(valid_dataset, batch_size = args.batch_size, drop_last = False, num_workers = args.workers,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Trianing the UNet model for segmentation of brain MRI',)
